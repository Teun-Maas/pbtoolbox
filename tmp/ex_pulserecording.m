...
% Speaker selection
	...
	speakers = speakers(~isnan(speakers(:,5)),:);

% Other parameters (for clarity)
	delay = 0; % [ms]
	atten = 0; % [dB]
% Prepare saving parameters
	fdir =['C:\DATA\JH\Click\' datestr(now,'yyyy-mm-dd') '\'];
	if ~exist(fdir,'dir')
		mkdir(fdir);
	end
...
for ii = 1:29
	...
	% Save the Sound inputs and outputs
		...
		fname = ['JH-' datestr(now,'yyyy-mm-dd') '-click-Spk' num2str(ii)];
...
figure(1)
clf
ax(1) = subplot(3,1,1); plot(T,Sounds.in); 
hold on
plot(T,Sounds.store); 
ax(2) =subplot(312); plot(T,Sounds.out); 
ax(3) =subplot(313);
