function ORIENT_JJH_CAL(varargin)
% PB_VGENEXP_VIS
%
...
% PBToolbox (2022): JJH: j.heckman@donders.ru.nl
   ...
   GV.ext                     = pb_keyval('extension',varargin,'.cal');                % Default .exp, for calibration set to .cal
   ...
   GV.exp_ntrials             = pb_keyval('ntrials',varargin,45);                      % Select the number of trails in each block (should be smaller than number of stimuli generated!)
   ...
   %     Stimuli
   GV.stim_repeats            = pb_keyval('repeats',varargin,3);                       % Keep at 1 or it wil repeat stimuli n times.
   GV.stim_duration           = pb_keyval('duration',varargin,1000);                   % Select the duration you want to present with. For multiple inset array (ex. [0.5 1 2 4 16]).
   ...
   GV.stim_ratiochair2world 	= pb_keyval('chair2world',varargin,[1 0]);               % Amount of chair fixed vs world fixed targets
   ...
   GV.vc_duration             = pb_keyval('vc_duration',varargin,0);                   % Duration (max duration of profile is 200s)
